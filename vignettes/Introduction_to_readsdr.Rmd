...
vignette: >
  %\VignetteIndexEntry{Introduction_to_readsdr}
...
we find specialised software such as [Vensim](http://vensim.com/) and 
...
contains the simulation parameters and the model variables as lists. From this
element, we can replicate the simulation period specified originally in 
Stella.
...
inputs but the integration method. deSolve returns a matrix which then is 
converted to a dataframe for convenience.
...
with the behaviour over time using ggplot2, a library part of the [tidyverse](www.tidyverse.org).
...
Finally, the third element from read_xmile is a list of two data frames. 
These structures describe the model as a graph in terms of vertices and edges,
and serve as input to the functions in [igraph](igraph.org), a library 
